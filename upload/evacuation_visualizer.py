...
class EvacuationVisualizer:
    ...
    def create_heatmap_animation(self):
        """Create heatmap animation showing crowd density over time with evacuated people in green"""
        ...
        for step in range(0, max_steps, max(1, max_steps // 100)):
            ...
            for person in self.people:
                ...
                if 0 <= x_idx < grid_size-1 and 0 <= y_idx < grid_size-1:
                    # Determine if person has reached safety by this step
                    if person.reached and step >= len(person.pos_history) - 1:
                        # Person has evacuated - show in GREEN
                        evacuated_density[y_idx, x_idx] += 1
                    else:
                        # Person is still evacuating - show in RED/YELLOW
                        active_density[y_idx, x_idx] += 1
            
            # Create frame with BOTH heatmaps
            traces = []
            
            # Layer 1: Active people (red/yellow) - BASE LAYER
            traces.append(go.Heatmap(
                ...
                colorscale='YlOrRd',
                showscale=True,
                hovertemplate='<b>Active People</b><br>Count: %{z}<extra></extra>',
                name='Active',
                colorbar=dict(
            ...
            # Layer 2: Evacuated people (green) - OVERLAY
            # Only show green where there are evacuated people
            ...
            traces.append(go.Heatmap(
                ...
                showscale=True,
                hovertemplate='<b>Evacuated (Safe)</b><br>Count: %{z}<extra></extra>',
                name='Evacuated',
                colorbar=dict(
        ...
        fig.update_layout(
            title={
                'text': '<b>Crowd Density Heatmap</b><br>'
                    '<sub>🔴 Red/Yellow = Active Evacuation | 🟢 Green = Reached Safe Zones</sub>',
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 18}
            },
        ...
        fig.update_layout(
            ...
            sliders=[{
                ...
                'steps': [{
                    ...
                    'label': f"{int(f.name) * self.config.time_scale:.0f}",
                    'method': 'animate'
                } for f in frames]
            }]
        )
